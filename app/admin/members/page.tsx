          ...
          {loading ? (
            ...
            <span className="flex flex-col gap-0.5">
              ...
              <span className="text-xs text-muted-foreground font-normal">
                {selected.ward.name} · {selected.ward.constituency.name} ·{" "}
                {selected.ward.constituency.county.name}
              </span>
            </span>
          ) : (
            "Select Stage…"
                ...
                >
                  ...
                  <div className="flex flex-col gap-0.5 min-w-0">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {s.ward.name} · {s.ward.constituency.name} ·{" "}
                      {s.ward.constituency.county.name}
                    </span>
                  </div>
                </CommandItem>
              ))}
...
function isStagePos(pos: string) {
  return STAGE_POSITIONS.has(pos);
}
...
function CreateMemberModal({
  ...
  useEffect(() => {
    ...
    stagesAdminApi
      ...
      .then((res) => {
        ...
        const arr: AdminStage[] =
          (res.data as { data?: AdminStage[] })?.data ?? [];
        setStages(arr);
        if (arr.length === 0) {
          setStagesError(
            "No stages registered yet. Add stages in Stages Management first.",
          );
        }
      })
      .catch((err: unknown) => {
        setStagesError(
          (err as { message?: string })?.message ?? "Failed to load stages",
        );
        setStages([]);
      })
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.phoneNumber.trim()) {
      setError(
        "Phone Number is required — the temporary password is sent to this number",
      );
      return;
    }
    if (!KENYA_PHONE_REGEX.test(form.phoneNumber.trim())) {
      setError(
        "Phone must be a valid Kenyan number (e.g. 0712345678 or 2547XXXXXXXX)",
      );
      return;
    }
    ...
    try {
      if (isStagePos(form.position)) {
        ...
        setError("Staff accounts are created from the Staff Users menu.");
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                The temporary password is sent to this number via SMS.
              </p>
            </div>
            ...
            {stageMember ? (
              ...
              <>
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    If blank, a system email will be generated
                  </p>
                </div>
            ...
            <div className="flex gap-3 pt-2 border-t">
              ...
              <Button
                ...
                className="flex-1"
                disabled={
                  loading || (stageMember && (!form.stageId || loadingStages))
                }
              >
                {loading ? (
                  <>
...
export default function MembersPage() {
  ...
  const membersPayload = membersQuery.data?.success
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
              ...
              <p className="text-sm text-muted-foreground">
                Create a member manually or import from CSV
              </p>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {canReveal && (
                              <DropdownMenuItem
                                onClick={() => setRevealMember(m)}
                              >
                                <KeyRound className="mr-2 h-4 w-4" /> Reveal
                                Temp Password
                              </DropdownMenuItem>
                            )}
                          </DropdownMenuContent>
